...
parser.add_argument("--network_type", "--nt", default="inception-resnet", choices=["inception-resnet"],
...
def main(options):
    ...
    if options.load is None:
        # Initial the model
        ## gen model
        if options.network_type == 'inception-resnet':
            model = inception_resnet.generate_model(2)
        ...
        for epoch_i in range(options.epochs):
            ...
            with torch.no_grad():
                ...
                if max_accuracy_val < val_avg_acu :
                    ...
                    torch.save(model.state_dict(), open('./MCIC' + ".nll_val_best_cv10", 'wb'))
